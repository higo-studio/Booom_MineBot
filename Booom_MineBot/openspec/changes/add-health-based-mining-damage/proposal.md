...
- **BREAKING**：恢复零风险安全墙的瞬时连锁开空，但只作为“首块墙体被击穿后的额外结算”；玩家或机器人当前直接命中的那面墙仍必须先走自己的生命/防御与破坏流程。
...
- `grid-mining-loop`: 挖掘成功条件改为“首块命中墙体基于生命、防御与矿机攻击力的持续伤害模型”，并在零风险安全墙被击穿后恢复额外的瞬时连锁开墙，同时新增中断宽限与回满规则。
