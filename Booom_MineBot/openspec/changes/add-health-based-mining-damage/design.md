...
- 现有“零风险安全墙连锁开格”如果整条链都绕过耐久，会冲掉当前需要保留的单墙破坏节奏与 crack 生命周期。
...
- 不把“零风险安全墙瞬时连锁”完全删除；本次改为“首块命中墙体先走耐久，击穿后再对连通安全区执行一次额外瞬时结算”的混合模型。
...
### 2. 用运行时墙体耐久状态替代首块墙体的瞬时 `TryMine()` 结算，并在击穿后恢复额外的安全墙连锁瞬开
...
6. 若该格不是炸弹，且其 `3x3` 炸药数为 `0`，则在同一次结算中向 `8` 邻接安全岩壁扩散并瞬时打开整片零风险区域；扩散遇到有数字的安全墙时打开该墙但停止继续外延

这里保留一个混合边界：当前直接命中的首块墙体必须独立走 `maxHealth / defense / crack` 生命周期；只有在它被击穿之后，零风险安全区才允许作为一次额外结算被瞬时打开。
...
- [恢复零风险连锁开墙会让连锁区域再次绕过逐墙耐久] → Mitigation：把边界明确收紧为“只允许首块命中墙体先走耐久，其余零风险安全墙作为额外瞬时结算”，并用回归测试锁定该分层行为。
...
2. 在 `GridMining` 内实现新的墙体耐久状态机，并恢复“首块墙体击穿后触发的零风险安全区瞬时连锁开墙”。
