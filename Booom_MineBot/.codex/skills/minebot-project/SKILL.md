...
- 当任务涉及 prefab 深改、UI 大重组或 PlayMode 截图/测试链路时，先看 `../unity-mcp-bridge/references/unity-mcp-quick-guide.md`，避免把 MCP 当成完整 Prefab 编辑器使用
- 跑 PlayMode 测试前，先确认 `unity.scene_list_opened` 中 open scenes 不脏；如果出现 `SaveCurrentModifiedScenesIfUserWantsTo()`，优先按编辑器状态问题处理
...
- `../unity-mcp-bridge/references/unity-mcp-quick-guide.md`
